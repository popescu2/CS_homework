...
-----------------------------  ProducerConsumer  ---------------------------------
  ...
  var
    ...
    --m_lock_full: Mutex = new Mutex
    --m_lock_empty: Mutex = new Mutex
...
-----------------------------  Dining Philosophers  ---------------------------------
  ...
  var
    l: int
    mon: ForkMonitor = new ForkMonitor
      con: array [5] of Condition = new array of Condition{5 of new Condition}
    m_lock1: Mutex = new Mutex
  ...
  class ForkMonitor
    ...
    methods
      ...
      PutDownForks (p: int)
      PrintAllStatus ()
      ForksAvailable (p: int) returns bool
  endClass
  ...
  behavior ForkMonitor
	...
	if m_lock1.IsHeldByCurrentThread() == false
		m_lock1.Lock()
	endIf
	...
	status[p] = HUNGRY
	mon.PrintAllStatus()
	while mon.ForksAvailable(p) == false
		--print("forksNotAvailable")
		--status[p] = HUNGRY
		--mon.PrintAllStatus()
		con[p].Wait(&m_lock1)		
	endWhile

--	print("ForksAvailable")
	...
	status[p] = THINKING
	mon.PrintAllStatus()
	if status[(p-2)%5] != EATING && status[(p-1)%5] == HUNGRY
		con[(p-1)%5].Signal(&m_lock1)
	else
		con[(p+1)%5].Signal(&m_lock1)
	endIf
	m_lock1.Unlock()
    ...
    method ForksAvailable(p: int) returns bool
	--Check
	if status[(p+1)%5] != EATING && status[(p-1)%5] != EATING
			return true
	endIf
