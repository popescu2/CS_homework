...
-----------------------------  InitFirstProcess  -------------------------------
  ...
  function InitFirstProcess ()
	--
	-- This routine initializes another process (besides the original process 
	-- which is running this code. It invokes GetANewThread to create a useable
	-- thread. The new thread will fork and run StartUserProcess. You can supply
	-- a zero as an argument to "StartUserProcess".
	var
		t: ptr to Thread
...
-----------------------------  StartUserProcess  ------------------------------
  ...
  function StartUserProcess(p: int)
	var
		pcb: ptr to ProcessControlBlock
		openFile: ptr to OpenFile
		initPC: int
		InitStackUserTop: int
		InitSystemStackTop: ptr to int
		OldIntStat: int
	-- get a new process with 'GetANew...' and initialize it.
	-- set the current thread's process to pcb, and the pcb's thread
	-- to the current Thread.

	-- set the openFile ptr to the return of the Open function
	-- initPC will be the starting point for the exec. 
	-- set pcb's address space by throwing it into the 'LoadExec...'
	-- function, which will set it automatically.
	...
	printInt(pcb.addrSpace.numberOfPages)
	nl()
	InitStackUserTop = ((pcb.addrSpace.numberOfPages) * PAGE_SIZE)
	
	InitSystemStackTop = & currentThread.systemStack[SYSTEM_STACK_SIZE -1]
	
	-- Set pagetable registers for the current thread??
	-- addrSpace.Init() was already called by the pcb.Init() call.

	pcb.addrSpace.SetToThisPageTable()
	-- frameManager.GetNewFrames(&pcb.addrSpace, 	
	--GetNewFrames was called in LoadExec...	
	
	currentThread.isUserThread = true

	-- The thread will never return after the next line.
	print("InitStackUserTop is: ")
	printInt(InitStackUserTop)
	nl()
	print("InitSystemStackTop is: ")
	...
	nl()
	--debug	
	BecomeUserThread(InitStackUserTop, initPC, InitSystemStackTop asInteger)

	-- Since the 'BecomeUser...' will set the interrupts to enabled
	-- I don't need to do it anymore
	-- OldInStat = SetInterruptsTo (OldIntStat)		
...
/*
	...
	-- to run the tests you guys needed.
	  ...
	  method Wait(mutex: ptr to Mutex)
		...
		if mutex.IsHeldByCurrentThread() == false
			mutex.Lock()
		endIf	
		--print("exiting wait...\n")
	  endMethod
	...
	-- to run the tests you guys needed.
	  ...
	  method Signal (mutex: ptr to Mutex)
        ...
        if t
        	...
        	t.status = READY
			mutex.heldBy = t
       		readyList.AddToFront (currentThread)
			oldIntStat = SetInterruptsTo (oldIntStat)
...
-----------------------------  ThreadManager  ---------------------------------
  ...
  behavior ThreadManager
      ...
      method Init ()
        ...
        --Init all the fields in Thread Manager
		...
		--set statuses and add each thread to the list of free threads.
		for i =0 to MAX_NUMBER_OF_PROCESSES -1
			threadTable[i].Init("Test Thread")
  ...
  function Handle_Sys_Shutdown ()
      -- NOT IMPLEMENTED
		print("Handle_Sys_Shutdown invoked!\n")
  		FatalError ("Syscall Shutdown was called by a user thread")
    endFunction
